	...
	submit.addEventListener("click", (e) => {
		...
		if (uname.length > 0) {
			if (pw == pwRepeat) {
				//add user content?
				console.log(
					"because of not knowing what to do here your data has not been saved lol :P"
				);
				window.location.href =
					"https://www.IcyTv.de/prijects/scribble-quickdraw/waiting"; //redirect to next page?
			} else {
				throw Error("passwords do not match");
			}
		} else if (uname) {
			//request saved usernames?
